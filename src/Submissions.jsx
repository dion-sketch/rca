...
function Submissions({ session, onBack, profileData }) {
  ...
  const fetchSubmissions = async () => {
    try {
      ...
      if (data) {
        setActiveSubmissions(data.filter(s => s.status !== 'completed' && s.status !== 'archived'))
      }
    } catch (err) {
      ...
      <div style={{ padding: '30px', maxWidth: '1000px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gap: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
              ...
              <div style={{
                ...
                <button disabled style={{ width: '100%', padding: '12px', borderRadius: '8px', border: 'none', backgroundColor: colors.gray, color: colors.background, fontSize: '14px', cursor: 'not-allowed', opacity: 0.5 }}>
                  Coming Soon
                </button>
              </div>
              ...
              <div style={{
                ...
                <button
                  ...
                  style={{ width: '100%', padding: '12px', borderRadius: '8px', border: 'none', backgroundColor: colors.primary, color: colors.background, fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}
                >
                  + Enter Opportunity
                </button>
              </div>
            ...
            <div style={{ backgroundColor: `${colors.primary}10`, borderRadius: '12px', padding: '15px 20px', border: `1px solid ${colors.primary}30`, marginTop: '10px' }}>
          ...
          <div style={{ display: 'grid', gap: '15px' }}>
            {activeSubmissions.length === 0 ? (
              <div style={{ backgroundColor: colors.card, borderRadius: '16px', padding: '40px', textAlign: 'center', border: `1px solid ${colors.gray}30` }}>
                ...
                <p style={{ color: colors.gray, margin: '0 0 20px 0' }}>Find an opportunity and start building your response!</p>
                <button onClick={() => setActiveTab('find')} style={{ padding: '12px 24px', borderRadius: '8px', border: 'none', backgroundColor: colors.primary, color: colors.background, fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}>
                  Find Opportunities
                </button>
              </div>
            ) : (
        ...
        {activeTab === 'completed' && (
          <div style={{ backgroundColor: colors.card, borderRadius: '16px', padding: '40px', textAlign: 'center', border: `1px solid ${colors.gray}30` }}>
            ...
            <p style={{ color: colors.gray, margin: 0 }}>Your submitted proposals will appear here.</p>
          </div>
        )}
      </div>
          ...
          <div style={{ backgroundColor: colors.card, borderRadius: '16px', padding: '30px', maxWidth: '500px', width: '100%', border: `2px solid ${colors.primary}`, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            {saveSuccess && savedOpportunity ? (
              <div style={{ textAlign: 'center' }}>
                ...
                <h3 style={{ color: colors.primary, margin: '0 0 10px 0', fontSize: '24px' }}>Found it!</h3>
                <p style={{ color: colors.white, margin: '0 0 5px 0', fontSize: '18px', fontWeight: '600' }}>{savedOpportunity.title}</p>
                <p style={{ color: colors.gray, margin: '0 0 25px 0', fontSize: '14px' }}>Added to Active • Due {new Date(savedOpportunity.due_date).toLocaleDateString()}</p>
                <div style={{ display: 'grid', gap: '10px' }}>
                  ...
                  <button onClick={handleViewInActive} style={{ width: '100%', padding: '14px', borderRadius: '8px', border: `1px solid ${colors.gray}`, backgroundColor: 'transparent', color: colors.white, fontSize: '14px', cursor: 'pointer' }}>
                    View in Active
                  </button>
              ...
              <>
                ...
                <p style={{ color: colors.gray, margin: '0 0 20px 0', fontSize: '13px' }}>Fill in what you know — only title and due date are required.</p>
                <div style={{ display: 'grid', gap: '15px' }}>
                  <div>
                ...
                <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '25px' }}>
                  ...
                  <button onClick={handleAddManual} style={{ padding: '12px 24px', borderRadius: '8px', border: 'none', backgroundColor: colors.primary, color: colors.background, cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>Save & Continue</button>
                </div>
              </>
            )}
          </div>
    ...
    return (
      <div style={{ minHeight: '100vh', backgroundColor: colors.background, fontFamily: 'Inter, system-ui, sans-serif' }}>
        <div style={{ backgroundColor: colors.card, padding: '20px 30px', borderBottom: `1px solid ${colors.primary}30` }}>
          <button onClick={onBack} style={{ background: 'none', border: 'none', color: colors.gray, cursor: 'pointer', fontSize: '16px' }}>← Back to Submissions</button>
        </div>
